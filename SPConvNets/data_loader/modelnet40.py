...
from .vgtk_utils import rotate_point_cloud, normalize_np, uniform_resample_np, rotation_distance_np, label_relative_rotation_np, label_relative_rotation_simple, get_anchorsV, get_anchors, get_relativeV_index, translate_point_cloud
from .gmm_noise import GmmSample
from SPConvNets.option import gen_options
...
class ModelNet40Alignment(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.opt.train_args.debug_mode == 'check_equiv':
            ...
            pc_src, R_src = rotate_point_cloud(pc, self.anchors[i])    # tmp!!!!
        else:
            pc_src, R_src = rotate_point_cloud(pc, max_degree = self.max_rotation_degree)
        ...
        in_dict =  {'pc1':torch.from_numpy(pc_src.astype(np.float32))[torch.randperm(pc_src.shape[0])],
                    'pc2':torch.from_numpy(pc_tgt.astype(np.float32))[torch.randperm(pc_tgt.shape[0])],
                    'fn': data['name'][0],
...
def gen_noisy_pc():
    opt = gen_options()
    opt.exp_args.dataset_path = "/home/rayzrzh/data/modelnet/EvenAlignedModelNet40PC"
    opt.exp_args.noise_augmentation = 0.01
    opt.exp_args.is_se3 = True
    opt.exp_args.outlier_augmentation = 0.01
    opt.batch_size = 1
    db_train = ModelNet40Alignment(opt)
    item = next(iter(db_train))
    pc1 = item['pc1']
    pc = o3d.geometry.PointCloud()
    pc.points = o3d.utility.Vector3dVector(pc1)
    o3d.io.write_point_cloud("noisy_pc1.ply", pc)

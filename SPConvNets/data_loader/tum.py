...
from .base import RegistrationDataset
import getpass
import ipdb
import numpy as np
import torch
...
from .vgtk_utils import rotate_point_cloud
import csv
import os
import cv2
import math
...
class TumFormatDataLoader(RegistrationDataset):
    ...
    def __init__(self,
        ...
        super(TumFormatDataLoader, self).__init__(name, run_mode)        
        ...
        if (self.kanchor == 12):
            ...
            self.anchors = L.get_anchors(self.kanchor)
    ...
    def _build_dataset(self):
        ...
        for scene in tqdm(sorted(scenes)):
            ...
            if scene != "/home/rayzhang/data/eth3d/training/table_3":
                continue
    ...
    def get_data_pair(self, scene_id, images_list, depths_list, poses_list, intrinsics_list, id1, id2):
        ...
        T = pose_f1_to_f2
        if self.kanchor == 12:
            R, R_label = label_relative_rotation_simple(self.anchors, T[:3,:3], self.rot_ref_tgt)
            if self.rot_ref_tgt:
            ...
            R, R_label = label_relative_rotation_np(self.anchors, T[:3,:3])
        ...
        xyz1_torch = torch.from_numpy(xyz1.astype(np.float32))[shuffle1]
                                      
        xyz2_torch = torch.from_numpy(xyz2.astype(np.float32))[shuffle2]
        in_dict = {
            'pc': torch.cat((torch.unsqueeze(xyz1_torch, dim=0),
                             torch.unsqueeze(xyz2_torch, dim=0)), dim=0),
            'pc1': xyz1_torch,
            'pc2': xyz2_torch,
            ...
            'seq': scene_id,
            'anchor_label':torch.from_numpy(label_anchor_aligned.astype(np.float32)) if self.kanchor == 12 else None,
            'pc1_id': id1,
            'pc2_id': id2,
            'intrinsic': K,
            'h': h,
    ...
    def __getitem__(self, index):
        if self.access_seq == 'random':
            ...
            return self.get_dataindex(self.dataset_index, index)
        else:
            assert self.access_seq in self.scene_info
            index = index % (len(self.scene_info[self.access_seq]['images'])-1)
            return self.get_seq_index(self.access_seq, self.scene_info[self.access_seq], index)

    def filter_nan_and_seq(self, target_seqs, is_filter_nan=False,
                           mode=''):
        new_data_index = []
        for seq_pair in tqdm(self.dataset_index):
            if seq_pair[0].split('/')[-1] in target_seqs:
                new_data_index.append(seq_pair)

        non_nan_index = []
        if is_filter_nan:
            for i in tqdm(len(dataset_index)):
                d = self.get_dataindex(new_data_index, i)
                if torch.isnan(d['pc1']) or \
                   ...
                   torch.isnan(d['R']):
                    continue
                non_nan_index.append(d)
            new_data_index = non_nan_index
                   
        print("new dataindex len is ", len(new_data_index))
        self.dataset_index = new_data_index
...
def filter_index(mode='train'):
    ...
    db_train.filter_nan_and_seq(target_seqs)

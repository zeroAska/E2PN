...
class RegistrationDataset(data.Dataset):
    ...
    def set_run_mode(self, mode : str):
        ...
        #self.seq_path_all = osp.join(self.root, mode_to_dir[mode])
    ...
    def __len__(self):
        return len(self.dataset_index)#sum([len(self.dataset_index[seq_key])for seq_key in self.dataset_index])
